...
public class DepartamentoListController implements Initializable, DataChangeListener {
	...
	public void onBtNovaAction(ActionEvent evento) {
		...
		/*
		 * Como é um botão para cadastrar um novo departamento o formulário vai começar
		 * vázio Então eu vou simplemente intstanciar um Departamento vázio E não vou
		 * colocar dado nele Só que vou injetar ele lá no controllador do Formulário.
		 */
	...
	private void incializarNodes() {
		...
		/*
		 * Crirei um objeto do tipo Stage Vou chamar a classe principal, e botar o
		 * metodo para puxar a cenaPrincipal e depois colocar o getWindow = função que
		 * pega a referencia para a janela. Ele Window é de uma SuperClasse do Stage
	...
	// Metodo auxiliar
	/*
	 * Vou colocar esse metodo recebendo como parametro uma referência para o Stage
	 * da janela que criou a janela de Dialogo. carregar janela para preencher um
	 * novo departamento vou ter chamar essa função, lá no botão onBtNewAction().
	 */
	private void criarDialogForm(Departamento obj, String nomeAbsoluto, Stage parentStage) {
		...
		try {
			...
			/*
			 * Injetando o Departamento Service depois que criei ele no
			 * DepartamentoFormController Ai também tenho que utilizar aqui pois se trata da
			 * janela. Injetando manualmente a depedência
			 */
			controller.setDepartamentoServico(new DepartamentoServico());
			// Escutando os eventos com o Observe
			// Do onDataChanged
			/*
			 * E com isso estou me inscrevendo para receber aquele evento que é
			 * onDataChanged utilizando this, que é a autoreferência.
			 */
			 ...
			 * Eu vou ter que instanciar um novo Stage ou seja vai ser um palco na frente do
			 * outro.
			 */
			...
			// função que pergunta quem é que é o Stage pai dessa janela ai que entra o
			// parametro 2.
	...
	private void initEditButtons() {
		...
		tabelaColunaEdit.setCellFactory(param -> new TableCell<Departamento, Departamento>() {
			...
			protected void updateItem(Departamento obj, boolean empty) {
				...
				setGraphic(button);
				button.setOnAction(
						event -> criarDialogForm(obj, "/gui/DepartamentoForm.fxml", Utils.palcoAtual(event)));
			}
		});
	}
